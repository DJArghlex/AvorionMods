...
function Scrapyard.allowWreckageYard()
	if allowWreckageYardForPlayers == false then
		-- the mod for allowing players/alliances to found scrapyards with wreckageyards isn't installed, but NPCs are still allowed to form wreckage yards.
		faction = Faction()
		...
		-- the mod for players/alliances to found scrapyards with wreckage yards is true, so just return true.
		return true
	end
end
...
function Scrapyard.initialize()
	...
	if onServer() then
		-- check to see if this station is going to allow a wreckage yard, and thus register the pile of callbacks that prevent players/alliances from stealing wrecks and damaging their reputations if they do
	
		if Scrapyard.allowWreckageYard() == true then
			-- vanilla code starts- registers callbacks for the scrapyard wreck claiming system
			local sector = Sector()
	...
	if onClient() and EntityIcon().icon == "" then
		-- set a different icon if this scrapyard is allowing wreckage yards
		if Scrapyard.allowWreckageYard() == true then
...
function Scrapyard.initializationFinished()
	...
	if onClient() then
		-- adjust the broadcasts so we're not lying to visitors about salvage licenses and whatnot.
		if Scrapyard.allowWreckageYard() == true then
...
function Scrapyard.initUI()
	...
	-- wrap salvage license code into a logic block so we're not generating this tab for non-wreckageyard-capable stations
	if Scrapyard.allowWreckageYard() == true then
...
function Scrapyard.onShowWindow()
	...
	-- wrap more salvage license code into a logic block so we're not showing this for non-wreckageyard-capable stations
	if Scrapyard.allowWreckageYard() == true then
		-- licenses
...
-- wrap into a logic block so we're not ticking code on the server every second for a scrapyard where it would be meaningless
...
-- wrap into a logic block so we're not ticking code on the client every second for a scrapyard where it would be meaningless
-- store function safely
...
-- any time this function gets called in a salvage sector a non-wreckageyard-capable station should simply return a one-hour salvage timer so that captained player/alliance ships can mine here. this function is called in their AIs when determining to salvage or not.
